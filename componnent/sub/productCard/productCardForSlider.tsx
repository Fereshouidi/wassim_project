...
const ProductCard = ({
    ...
    return (
        <motion.div
            ...
            variants={fadeInUp}
            whileTap={{ scale: 0.98 }}
            className={`flex relative flex-col items-center justify-between gap-2 rounded-lg overflow-hidden cursor-pointer py-1 px-1 sm:py-2 sm:px-2 transition-all duration-300 ${className}`}
            style={{
                ...style,
        ...
        >
            {useLike && <div
                className={`absolute top-0 right-0 sm:top-0 sm:right-0 rounded-full- p-[5px] ${like ? "bg-red-500" : "bg-gray-400 opacity-75"} transition-transform active:scale-75 w-8 h-8 z-[2] cursor-pointer`}
                onClick={(e) => {
                    e.stopPropagation();
                    if (client) {
                        setLike(!like);
            ...
            <div
                className='w-full h-[150px] sm:h-[280px] pt-2- bg-red-500- rounded-lg overflow-hidden'
            ...
            >
                {
                    (product.mainImageSource === 'firstSpecification' ? (product.images?.[0]?.uri || product.thumbNail) : product.thumbNail) ? <img
                        ...
                        className='w-full h-full rounded-lg overflow-hidden hover:scale-110 duration-300 object-cover'
                        alt={product.name[activeLanguage.language] || ""}
                    /> : <SkeletonLoading />
                }
            </div>

            <h4 className={`w-full h-7 flex items-center justify-center rounded-lg overflow-hidden z-50- text-[14px] sm:text-[16px] text-center px-4 font-medium`}>
                {product.name?.[activeLanguage.language]
                    ? handleLongText(product.name[activeLanguage.language] + "", 14)
                    : <div className="w-28 h-4 opacity-50"><SkeletonLoading /></div>
                }
            </h4>

            <div className={`w-fit min-w-[90px] h-8 overflow-hidden rounded-lg flex flex-row items-center justify-center gap-2 px-2`}>
