...
const ProductCard = ({ product, className, style, useLike }: ProductCardType) => {
        ...
        return (
            <>
                ...
                <div className={`w-full flex ${isMob ? 'flex-col' : 'justify-between'} items-center gap-2 p-2 border-t border-gray-100`}>
                    <button
                        className={`flex items-center justify-center gap-2 flex-1 py-2.5 rounded-lg transition-all cursor-pointer w-full ${isInCart ? 'bg-green-50' : 'hover:bg-gray-100'}`}
                    ...
                    <button className="flex items-center justify-center gap-2 flex-1 py-2.5 rounded-lg cursor-pointer w-full" style={{ backgroundColor: colors.dark[200], color: colors.light[200] }}>
    ...
    return (
        <motion.div
            ...
            variants={fadeInUp}
            whileTap={{ scale: 0.98 }}
            className={`group relative flex flex-col justify-between overflow-hidden rounded-lg transition-all duration-500 ${className} ${isMobile ? 'w-full h-[380px]' : 'w-full max-w-[320px] min-h-[400px]'} cursor-pointer`}
            style={{
                ...style,
